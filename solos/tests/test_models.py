...
class SoloModelTestCase(TestCase):
    ...
    def test_get_absolute_url(self):
        """
        Test that we can build an url for a solo
        """
        self.assertEqual(self.solo.get_absolute_url(), '/recordings/at-the-stratford-shakespearean-festival/'
                                                       'falling-in-love-with-love/oscar-peterson/')

    def test_get_duration(self):
        """
        Test that we can print duration for a Solo
        """
    ...
    def test_get_artist_tracks_from_musicbrainz(self, mock_mb_search_artists, mock_mb_browse_releases):
        ...
        # setting a couple recordings to avoid too much nesting
        recording1 = {
            'recording': {
